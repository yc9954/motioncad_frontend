        ...
        configure: (proxy, _options) => {
          ...
          proxy.on('proxyRes', (proxyRes, req, res) => {
            ...
            if ([301, 302, 307, 308].includes(proxyRes.statusCode || 0)) {
              ...
              else if (location && location.includes('ec2-54-180-23-126.ap-northeast-2.compute.amazonaws.com')) {
      ...
      '/login/oauth2': {
        ...
        configure: (proxy, _options) => {
          ...
          proxy.on('proxyRes', (proxyRes, req, res) => {
            ...
            if ([301, 302, 307, 308].includes(proxyRes.statusCode || 0)) {
              const location = proxyRes.headers['location'];
              if (location && location.includes('ec2-54-180-23-126.ap-northeast-2.compute.amazonaws.com')) {
                const url = new URL(location);
                const newLocation = url.pathname + url.search;
                if (!res.headersSent) {
                  proxyRes.headers['location'] = newLocation;
